...
-(void) fixRegionAndAccesoryViews
{
    ...
    if (self.bounds.size.width != self.contentView.bounds.size.width && [self isRTLLocale]) {
         _swipeOverlay.frame = CGRectMake(-self.bounds.size.width + self.contentView.bounds.size.width, 25, _swipeOverlay.bounds.size.width, _swipeOverlay.bounds.size.height - 45);
    }
}
...
-(void) layoutSubviews
{
    ...
    if (_swipeOverlay) {
        ...
        _swipeOverlay.frame = CGRectMake(0, 25, self.bounds.size.width, self.contentView.bounds.size.height - 45);
...
-(void) createSwipeViewIfNeeded
{
    UIEdgeInsets safeInsets = [self getSafeInsets];
    if (!_swipeOverlay) {
        _swipeOverlay = [[UIView alloc] initWithFrame:CGRectMake(0, 20, self.bounds.size.width, self.contentView.bounds.size.height - 40)];
        [self fixRegionAndAccesoryViews];
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    ...
    if (gestureRecognizer == _panRecognizer) {
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(swipeTableCell:canSwipe:fromPoint:)]) {
           ...
           CGPoint velocityInView =  [_panRecognizer velocityInView:self];
